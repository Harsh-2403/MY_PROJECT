...
if(isset($_POST['register']))
{
	...
	$query1 = "SELECT phone_number FROM tbl_student WHERE phone_number = '$phone'";
	$fire = mysqli_query($conn,$query1) or die("can not fire query to select phone number".mysqli_query($conn,$fire));

	if(mysqli_num_rows($fire)>0)
	{
		echo "<script>alert('phone number already existed register with other phone number')</script>";
	}
	else
	{
		$digits = 6;
		...
		header("Location:verify_otp.php?user=".$phone);
		exit();
		}
		else
		{
...
<body>
	...
	<div class="container"> <h2>Registration Form</h2> 
		<div class="form-container">

			<form action="" method="POST">
				<div class="input-name">
					<i class="fa fa-user lock"></i>
					<input type="text" placeholder="Enter First Name" id="f_name" class="name" name="fname" required title="Enter Your First Name">
					<span> 
						<i class="fa fa-user lock"></i>
						<input type="text" placeholder="Enter Last name" class="name" name="lname" required title="Enter Your Last Name">
					</span>
				</div>

				<div class="input-name">
					<i class="fa fa-envelope  email"></i>
					<input type="email" id="mail" placeholder="Enter Your Email Address" class="text-name" name="mail" title="Enter Your Email Address" required oninput="this.setCustomValidity('')"oninvalid="this.setCustomValidity('please enter a valid email address')"/>
				</div>

				<div class="input-name">
					<i class="fa fa-lock lock"></i>
					<input type="Password" placeholder="Enter Your Password" class="text-name" name="password" id="myinput" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"  title="Must contain at least one number and one uppercase and lowercase letter and one special character, and at least 8 or more characters" title="Enter Your Password" required oninput="this.setCustomValidity('')"
					oninvalid="this.setCustomValidity('Password Must Be Required contain at least one number and one uppercase and lowercase letter one special character and at least 8 or more characters')"/>
	...
	<div class="input-name" >
		<input type="submit" class="button" value="submit" name="register" onclick="myFunction()"/>
		<script>
			function myFunction()
			{
